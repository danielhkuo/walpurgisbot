    ...
    async def daily_check(self):
        ...
        if not latest_day or latest_day < expected_day:
            try:
                ...
                await channel.send(f"<@{JOHAN_USER_ID}> Reminder: You haven't added Daily Johan for day {expected_day} yet!")
        ...
        if latest_day < expected_day - 1:
            try:
                await channel.send(f"Alert: There seems to be a gap in Daily Johans. Last recorded day was {latest_day}.")
            except Exception as e:
